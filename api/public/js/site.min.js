async function fetchResumeData(lang) {
  try {
    const response = await fetch(
      "https://raw.githubusercontent.com/joaojkuligowski/joaopedro.site/da9faf0f239ae96ef0ee36209ba8431d3c9c3c92/api/public/lang/" +
        lang +
        ".json"
    );
...
function renderResume(data) {
  ...
  <div class="bg-white rounded-lg shadow-lg p-2">
      ...
      <hr class="my-2">
      <div class="flex flex-col sm:flex-row items-center sm:items-start mb-6 space-y-4 sm:space-y-0">
        <img src="${
          data.profilePicture
        }" alt="Foto de Perfil" class="w-16 h-16 sm:w-24 sm:h-24 rounded-full mr-0 sm:mr-4">
        
        <div class="flex-1 text-center sm:text-left">
          <h1 class="text-xl sm:text-2xl font-bold">${data.name}</h1>
          <p class="text-gray-700">${data.position}</p>
          <p class="text-gray-700">Phone: +55 48 991160433</p>
          <p class="text-gray-700">
            Email: 
            <a href="mailto:joaojkuligowski@proton.me" class="text-blue-500">joaojkuligowski@proton.me</a>
          </p>
        </div>
        
        <div class="sm:ml-auto mt-4 sm:mt-0">
          <button id="btn-print" onclick="preparePrint()" class="w-full sm:w-auto cursor-pointer bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
            Imprimir
          </button>
        </div>
      </div>
      ...
      <nav class="w-full mb-8" id="navigation">
        <ul class="flex justify-center space-x-8 text-gray-700 font-semibold text-lg">
          ${data.menu
            ?.map(
              (item) =>
                `<li route-link="${item.route}" class="hover:text-blue-500 transition-colors duration-300"><a href="${item.route}">${item.text}</a></li>`
            )
            .join("")}
        </ul>
      </nav>
      ...
      <ul>
        ${data.experience
          ...
          <li class="mb-4">
            ...
            <p class="text-gray-600">${job.company} | ${job.location} | ${
              job.period
            }</p>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <h4 class="font-medium mb-1">${
            data.sections.programmingLanguages
          }</h4>
          <p class="text-gray-700">${data.skills.programmingLanguages.join(
            ", "
          )}</p>
        </div>
        <div>
          <h4 class="font-medium mb-1">${
            data.sections.frameworksAndLibraries
          }</h4>
          <p class="text-gray-700">${data.skills.frameworksAndLibraries.join(
            ", "
          )}</p>
        </div>
        <div>
        ...
        <div>
          <h4 class="font-medium mb-1">${
            data.sections.toolsAndTechnologies
          }</h4>
          <p class="text-gray-700">${data.skills.toolsAndTechnologies.join(
            ", "
          )}</p>
        </div>
        <div>
      ...
      <p class="text-gray-700">${data.education.degree} | ${
    data.education.institution
  } | ${data.education.period}</p>
